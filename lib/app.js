 ...
 */
(function() {
    var app = angular.module('tronchotron', ['navCtrl','ui.bootstrap','cgPrompt']);
    ...
    app.controller('mainCtrl', ['$scope','$http','$log',
        function($scope,$http,$log) {
            ...
            $scope.isSetReloadTronche = function(){
            ...
            angular.forEach(window.localStorage, function(value, key) {
                ...
                index++;
                $log.debug(reloadedTronche)
                $scope.reloadedTronches.push(reloadedTronche);
            });
            $log.debug($scope.reloadedTronches)
            $scope.getIndex = function(){
                var index = (angular.element('#reloadIndex').html()).substr(0,1);
                return index;
            };
